...
class TRTInference:
    ...
    def display_recognized_image(self, image_path, class_label):
        ...
        for class_name in class_label:
            ...
            path_to_detected_img = "/deeplearning/resnet/multiple_images_resnet"
        
            if not os.path.exists(path_to_detected_img):
            
                os.makedirs(path_to_detected_img)
            
            '''save_img = os.path.join(path_to_detected_img, f'{class_img }.jpeg')
        
                plt.savefig(save_img)
        
            plt.close()'''
        # ensure the class label is a valid filename
       # safe_class_label = "".join([c for c in class_label if c.isalpha() or c.isdigit() or c==' ']).rstrip()
            ...
            plt.axis('off')
            save_img = os.path.join(path_to_detected_img, f'{class_name}.jpeg')
